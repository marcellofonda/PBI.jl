...
# Material parameters
δ = 2.3e-5 #Adimensional
β = 4.25e-10 #Adimensional
...
tetta = [max_thickness / (1+ exp((sqrt(i^2 + j^2) - pixelradius+5pixelborder)/pixelborder)) for i in -pixelradius:pixelradius, j in -pixelradius:pixelradius]
...
# Extract images
I_0_m, paths =  DarkFieldRetrieve(downsampled, δ, β, R, 2π/k, pixelsize * downscale_factor, 5e-2; scale=1)
